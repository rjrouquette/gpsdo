...
//    }
    ...
    float del[2], dif[2];
    del[0] = (temp - mat[0]) / (float) initCnt;
    del[1] = (comp - mat[1]) / (float) initCnt;
    mat[0] += del[0];
    mat[1] += del[1];
    if(initCnt < 2) {
        return;
    ...
    dif[0] = temp - mat[0];
    dif[1] = comp - mat[1];
    mat[2] += ((dif[0] * dif[0]) - mat[2]) / (float) initCnt;
    mat[2] += del[0] * del[0];
    mat[3] += ((dif[0] * dif[1]) - mat[3]) / (float) initCnt;
    mat[3] += del[0] * del[1];
}
