...
struct [[gnu::packed]] PTP2_SRC_IDENT {
    uint8_t identity[8];
    uint16_t portNumber;
};

static_assert(sizeof(PTP2_SRC_IDENT) == 10, "PTP2_SRC_IDENT must be 10 bytes");

struct [[gnu::packed]] PTP2_TIMESTAMP {
    uint16_t secondsHi;
    uint32_t secondsLo;
    uint32_t nanoseconds;
};

static_assert(sizeof(PTP2_TIMESTAMP) == 10, "PTP2_TIMESTAMP must be 10 bytes");

struct [[gnu::packed]] HEADER_PTP {
...
static_assert(sizeof(PTP2_ANNOUNCE) == 30, "PTP2_ANNOUNCE must be 34 bytes");

struct [[gnu::packed]] PTP2_DELAY_RESP {
    PTP2_TIMESTAMP receiveTimestamp;
    PTP2_SRC_IDENT requestingIdentity;
};

static_assert(sizeof(PTP2_DELAY_RESP) == 20, "PTP2_DELAY_RESP must be 34 bytes");

struct [[gnu::packed]] PTP2_PDELAY_REQ {
    PTP2_TIMESTAMP receiveTimestamp;
    uint8_t reserved[10];
};

static_assert(sizeof(PTP2_PDELAY_REQ) == 20, "PTP2_PDELAY_REQ must be 34 bytes");

struct [[gnu::packed]] PTP2_PDELAY_RESP {
    PTP2_TIMESTAMP receiveTimestamp;
    PTP2_SRC_IDENT requestingIdentity;
};

static_assert(sizeof(PTP2_PDELAY_RESP) == 20, "PTP2_PDELAY_RESP must be 34 bytes");

struct [[gnu::packed]] PTP2_PDELAY_FOLLOW_UP {
    PTP2_TIMESTAMP responseTimestamp;
    PTP2_SRC_IDENT requestingIdentity;
};

static_assert(sizeof(PTP2_PDELAY_RESP) == 20, "PTP2_PDELAY_FOLLOW_UP must be 34 bytes");
...
struct PacketPTP {
    ...
    static auto& from(void *frame) {
        return *static_cast<PacketPTP<T>*>(frame);
    }

    static auto& from(const void *frame) {
        return *static_cast<const PacketPTP<T>*>(frame);
    }
};

static_assert(offsetof(PacketPTP<uint64_t>, data) == PacketPTP<uint64_t>::DATA_OFFSET, "PacketPTP.data is misaligned");
